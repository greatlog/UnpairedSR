...
class KernelModel(nn.Module):
    ...
    def forward(self, x):
        ...
        kernel = self.deg_kernel(inp).view(B, 1, ksize**2, *zk.shape[2:])
...
class NoiseModel(nn.Module):
    ...
    def cal_noise(self, x):
        ...
        noise_std = self.deg_noise(inp)
        noise_std = torch.exp(0.5 * noise_std)

        noise = torch.randn_like(noise_std)
        minimum = -x.detach() / noise_std.detach()
        maximum = (1 - x.detach()) / noise_std.detach()
        noise = torch.where(
            torch.gt(noise, minimum), noise, minimum
            )
        noise = torch.where(
            torch.gt(noise, maximum), maximum, noise
            )
