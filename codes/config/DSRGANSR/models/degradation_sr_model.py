...
class DegSRModel(BaseModel):
        ...
        if self.is_train:
            ...
            for name in defined_optimizer_names:
                ...
                self.optimizers[name] = self.build_optimizer(
                    self.networks[name], optim_config["optim"]
                )

                self.schedulers[name] = self.build_scheduler(
                    self.optimizers[name], optim_config["scheduler"]
                )
    ...
    def optimize_trans_models(self, loss_dict, step):
        ...
        self.set_requires_grad(["Decoder"], False)
        ...
        self.update_learning_rate(["Encoder"], step)
    ...
    def optimize_sr_models(self, loss_dict, step):
        ...
        self.set_requires_grad(["Encoder"], False)
        ...
        self.update_learning_rate(["Decoder"], step)
    ...
    def test(self, test_data):
        ...
        self.set_network_state(["Encoder", "Decoder"], "eval")
        with torch.no_grad():
            self.fake_tgt = self.Decoder(self.src)
            if test_data.get("tgt") is not None:
                self.fake_lr = self.Encoder(self.tgt)
        self.set_network_state(["Encoder", "Decoder"], "train")
