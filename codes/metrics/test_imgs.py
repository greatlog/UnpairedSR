...
from .messure import IQA


def parse_argumnets():
    parser = argparse.ArgumentParser()
    parser.add_argument("--res_dir", type=str, default=None, help="directory of test images")
    parser.add_argument("--ref_dir", type=str, default=None, help="directory of reference images")
    parser.add_argument("--save_dir", type=str, default=".", help="directory of saved results")
    parser.add_argument("--metrics", type=list, default=["psnr, ssim, lpips"])
...
def main():
    ...
    if args.res_dir is None:
        raise TypeError(
            "res dir can not be None"
        )
        if not osp.exists(args.res_dir):
            raise ValueError(
                "res dir dose not exist"
            )
    ...
    messure = IQA(metircs = args.metircs, cuda=False)
    ...
    for indx, res_path in enumerate(res_paths):
        ...
        scores = messure(res = res_img, ref=ref_img, metrics=args.metrics)
        ...
        if res_img.ndim == 3:
            ...
            message += "Y Scores\t"
            scores = measure(res=res_img_y, ref=ref_img_y, metircs=args.metrics)
            for k, v in scores.items():
